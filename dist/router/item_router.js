...
import { decryptData, verifyToken, } from "../utils/utils.js";
...
console.log("---------------------------------------------------");
console.log("GEMINI API KEY 확인:", apiKey ? "키가 존재합니다 (OK)" : "!!! 키가 없습니다 (NULL) !!!");
console.log("---------------------------------------------------");
...
router.get("/get_items", async (c) => {
    ...
    try {
        ...
        if (searchKeyword?.length > 0) {
            try {
                const embedRes = await fetch("https://wildojisan-embeddinggemma-300m-fastapi.hf.space/make_text_embedding", {
                    method: "POST",
...
router.get("/get_item_by_id", async (c) => {
  ...
  `;
        let _result = await db.query(updateQuery, [item_id]);
...
router.get("/get_categories", async (c) => {
    ...
    FROM t_category
  `;
        let _result = await db.query(query, []);
        _result = _result?.rows || [];
        result.data = _result;
        return c.json(result);
    }
...
router.post("/upsert_item", async (c) => {
  ...
  `;
        ...
        if (files) {
            ...
            uploadResults = await Promise.all(files.map(async (file) => {
                try {
